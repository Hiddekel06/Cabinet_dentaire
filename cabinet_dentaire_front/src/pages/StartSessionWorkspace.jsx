...
const StartSessionWorkspace = () => {
  ...
  // États pour l'accord financier global
  const [inlineAgreedAmount, setInlineAgreedAmount] = useState('');
  const [inlineAgreedDate, setInlineAgreedDate] = useState(new Date().toISOString().slice(0, 10));
  const [savingAgreed, setSavingAgreed] = useState(false);

  useEffect(() => {
    if (treatment) {
      setInlineAgreedAmount(treatment.agreed_amount || '');
      setInlineAgreedDate(treatment.agreed_amount_date ? new Date(treatment.agreed_amount_date).toISOString().slice(0, 10) : new Date().toISOString().slice(0, 10));
    }
  }, [treatment]);

  const hasAccordChanged = useMemo(() => {
    const originalAmount = treatment?.agreed_amount || '';
    const originalDate = treatment?.agreed_amount_date ? new Date(treatment.agreed_amount_date).toISOString().slice(0, 10) : new Date().toISOString().slice(0, 10);
    return String(inlineAgreedAmount) !== String(originalAmount) || String(inlineAgreedDate) !== String(originalDate);
  }, [treatment, inlineAgreedAmount, inlineAgreedDate]);
  ...
  const handleSaveInlineAgreed = async () => {
    if (inlineAgreedAmount && isNaN(Number(inlineAgreedAmount))) {
      alert('Veuillez entrer un montant valide.');
      return;
    }

    const todayStr = new Date().toISOString().slice(0, 10);
    if (inlineAgreedDate > todayStr) {
    ...
    try {
      const res = await patientTreatmentAPI.update(treatmentId, {
        agreed_amount: inlineAgreedAmount ? Number(inlineAgreedAmount) : null,
        agreed_amount_date: inlineAgreedAmount ? inlineAgreedDate : null,
      });
      setTreatment(res.data?.data || res.data || null);
      showFeedback('success', 'Accord financier mis à jour', 'L\'accord financier global a été enregistré avec succès.');
    } catch (err) {
      console.error(err);
      alert('Erreur lors de la sauvegarde de l\'accord.');
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validation préalable du montant convenu si modifié
    if (hasAccordChanged) {
      if (inlineAgreedAmount && isNaN(Number(inlineAgreedAmount))) {
        alert('Veuillez entrer un montant convenu valide.');
        return;
      }
      const todayStr = new Date().toISOString().slice(0, 10);
      if (inlineAgreedDate > todayStr) {
        alert('La date de l\'accord ne peut pas être dans le futur.');
        return;
      }
    }
    ...
    try {
      ...
      // Préparation du payload de mise à jour du traitement
      const treatmentUpdatePayload = {
      ...
      // Si l'accord financier a changé, on l'enregistre en même temps
      if (hasAccordChanged) {
        treatmentUpdatePayload.agreed_amount = inlineAgreedAmount ? Number(inlineAgreedAmount) : null;
        treatmentUpdatePayload.agreed_amount_date = inlineAgreedAmount ? inlineAgreedDate : null;
      }

      await patientTreatmentAPI.update(treatment.id, treatmentUpdatePayload);
            ...
            <div className="p-6 space-y-6">
              ...
              <section className="space-y-4 pt-6 border-t border-gray-100">
                ...
                <div className="bg-emerald-50/50 rounded-2xl p-5 border border-emerald-100 space-y-4">
                  <div>
                    <label className="block text-xs font-bold text-emerald-800 uppercase tracking-wide mb-2">Montant encaissé sur cette séance (XOF)</label>
                    ...
                    {collectedSoFar > 0 && (
                      <p className="mt-2 text-xs font-semibold text-emerald-700 flex items-center gap-2">
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                          <path d="M7 3a1 1 0 000 2h6a1 1 0 100-2H7zM4 7a1 1 0 011-1h10a1 1 0 110 2H5a1 1 0 01-1-1zM2 11a2 2 0 012-2h12a2 2 0 012 2v4a2 2 0 01-2 2H4a2 2 0 01-2-2v-4z" />
                        </svg>
                        Déjà réglé sur ce traitement : {collectedSoFar.toLocaleString('fr-FR')} XOF
                      </p>
                    )}
                  </div>
                  ...
                  <div className="bg-white rounded-xl p-4 border border-slate-200/80 shadow-sm space-y-3">
                    <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                      ...
                      {treatment?.agreed_amount && (
                        <span className="text-[10px] text-slate-500 font-medium">
                          Fixé le {new Date(treatment.agreed_amount_date).toLocaleDateString('fr-FR')}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        ...
                        <div className="relative">
                          <input
                            ...
                            step="0.01"
                            min="0"
                            value={inlineAgreedAmount}
                            onChange={(e) => setInlineAgreedAmount(e.target.value)}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={inlineAgreedDate}
                          onChange={(e) => setInlineAgreedDate(e.target.value)}
                    ...
                    {inlineAgreedAmount && !isNaN(Number(inlineAgreedAmount)) && (
                      <div className="bg-indigo-50/50 rounded-lg p-2.5 border border-indigo-100/50 text-[11px] text-indigo-950 flex justify-between items-center">
                        <div>
                          ...
                          <span className="font-bold text-indigo-700">
                            {Math.max(0, Number(inlineAgreedAmount) - collectedSoFar).toLocaleString('fr-FR')} XOF
                          </span>
                        </div>
                        {hasAccordChanged && (
                          <button
                            type="button"
                            onClick={handleSaveInlineAgreed}
                            disabled={savingAgreed}
                            className="px-2.5 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded text-[10px] font-bold transition-all disabled:opacity-50 cursor-pointer"
                          >
                            {savingAgreed ? '...' : 'Enregistrer'}
                          </button>
                        )}
                      </div>
                    )}
                    
                    {!inlineAgreedAmount && hasAccordChanged && (
                      <div className="flex justify-end">
                        <button
                          type="button"
                          onClick={handleSaveInlineAgreed}
                          disabled={savingAgreed}
                          className="px-2.5 py-1 bg-rose-600 hover:bg-rose-700 text-white rounded text-[10px] font-bold transition-all disabled:opacity-50 cursor-pointer"
                        >
                          {savingAgreed ? '...' : "Supprimer l'accord"}
                        </button>
                      </div>
                    )}
                  </div>
